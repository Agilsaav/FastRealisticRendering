...
uniform sampler2D saao_x;
//uniform sampler2D noiseText;

uniform vec3 samples[64];

// parameters 
uniform int samplesY;
...
void main()
{
	...
	float AOx = texture(saao_x, TexCoords).r;

	vec3 dirvec = vec3(0.0, 1.0, 0.0); //Direction vector for the y calculations
	...
	for (int i = 0; i < samplesY; ++i)
	{
		...
		vec3 dir_sample = (dirvec * samples[i].y);
		...
		vec3 sample = fragPos + (dir_sample * radius);
	...
	occlusion = 1.0 - (occlusion / samplesY);

	FragColor = (AOx + occlusion)/2.0;
}
